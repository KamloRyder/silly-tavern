...
class MVUService {
   ...
   * @param messageId 消息楼层 ID
   */
  async updateGameMode(mode: 'main' | 'combat' | 'interaction' | 'creation', messageId: number = 0): Promise<void> {
    this.ensureInitialized();
    ...
    try {
      ...
      data.game.mode = mode;
      await this.saveGameData(data, messageId);
      console.log(`[MVU Service] 游戏模式更新为: ${mode}`);
    } catch (error) {
  ...
  async setCurrentInstance(instanceId: string, messageId: number = 0): Promise<void> {
    ...
    try {
      ...
      data.instances[instanceId].isActive = true;

      // 将其他副本设为非活跃
      Object.keys(data.instances).forEach(id => {
        if (id !== instanceId) {
          data.instances[id].isActive = false;
        }
      });
